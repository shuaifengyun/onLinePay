...
int g_CryptoTaOther_Compare(CHAR* oneBuf, CHAR* twoBuf, UINT32 len);
UINT32 g_CryptoTaOther_mixData(CHAR* oneBuf, UINT32 oneLen, CHAR* twoBuf, UINT32 twoLen, CHAR* pOutBuf);
...
int g_CryptoTaOther_Compare(CHAR* oneBuf, CHAR* twoBuf, UINT32 len)
{
    ...
    {
        if(oneBuf[l_Index] != twoBuf[l_Index])
        {
            return FAIL;
        }
    }
...
UINT32 g_CryptoTaOther_mixData(CHAR* oneBuf, UINT32 oneLen, CHAR* twoBuf, UINT32 twoLen, CHAR* pOutBuf)
{
    ...
    TEE_MemMove(&(pOutBuf[l_loopNum * 2U]), &(l_pTem[l_loopNum]), (twoLen - l_loopNum + 1));
